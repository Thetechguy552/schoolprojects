...
drop schema if exists csd220_tennis_club_v2;
create schema csd220_tennis_club_v2 character set utf8mb4;
...
-- Rename the recreational_member table
alter table recreational_member 
rename `member`
;

-- Bring the competitive member info over into the member table
insert into `member` 
	select player_number, first_name, last_name, sex, year_joined, phone, email
	from competitive_member
;

-- Turn the player_number field in competitive_member into a foreign key column
-- that points to the member table
alter table competitive_member 
	add foreign key (player_number) references `member`(player_number)
;

-- Get rid of the now-redundant rows in the competitive_members table
alter table competitive_member 
...
update coach_assignment set coach_player_number = 105 
where coach_player_number = 102
;

-- TASK 1.2

alter table `member` add column active boolean not null default 1;


update `member` set active = 0 where year_joined < (year(now()) - 5);